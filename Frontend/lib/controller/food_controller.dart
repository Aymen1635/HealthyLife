...
class FoodController {
  ...
  Future<List<Food>> fetchFoods() async {
    final response =
        await http.get(Uri.parse('http://192.168.1.115:8000/foods'));
  ...
  Future<List<Food>> fetchFoodsByType(String type) async {
    final response = await http.get(Uri.parse(
        'http://192.168.1.115:8000/foods/api/foods/$type')); //! ToDo: /api rootlari kaldirilmali
  ...
  Future<Food> createFood(Food food) async {
    final response = await http.post(
      Uri.parse('http://192.168.1.115:8000/api/foods'),
